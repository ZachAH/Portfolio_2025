...
export default function BusinessTemplate2({ handleMouseEnter, handleMouseLeave }) {
  ...
  return (
    <div 
      className="interactive-element bg-zinc-900/50 backdrop-blur-md border border-zinc-800 rounded-3xl overflow-hidden hover:border-[#4F5748]/50 transition-all duration-500 group shadow-2xl"
    ...
    >
      ...
      <div className="relative h-72 w-full overflow-hidden bg-zinc-950 border-b border-zinc-800">
        ...
        />
        {/* Floating Price Tag - Forest Green Accent */}
        <div className="absolute top-6 right-6 bg-[#4F5748] text-white px-4 py-1.5 text-xs font-black rounded-full uppercase tracking-widest shadow-[0_0_20px_rgba(79,87,72,0.4)]">
          $59 - Source Code
        </div>
      </div>
      ...
      {/* Content Section */}
      <div className="p-10">
        <h3 className="text-4xl font-bold text-white mb-4 tracking-tight group-hover:text-[#4F5748] transition-colors duration-300">
          Universal Business
        </h3>
        ...
        {/* Tech Tags - Zinc/Green Accents */}
        <div className="flex flex-wrap gap-2 mb-10">
          {['React', 'Vite', 'Tailwind', 'Universal Layout'].map((tech) => (
            <span key={tech} className="text-[10px] uppercase tracking-widest px-3 py-1 rounded-md bg-zinc-800/50 text-zinc-300 border border-zinc-700 group-hover:border-[#4F5748]/30 transition-colors duration-500">
              {tech}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-5">
          <a 
            ...
            className="flex-1 border-2 border-[#4F5748]/80 text-[#4F5748] font-black py-3 px-6 rounded-xl hover:bg-[#4F5748] hover:text-white transition-all duration-300 text-center uppercase tracking-widest text-sm"
          >
            Preview
          </a>
